...
- **配置反向代理**：按域名生成站点配置，支持 `http://127.0.0.1:8080` 本机后端，也支持 `https://源站域名` 回源（自动补 SNI 与源站 Host，避免串站；源站是公网可信证书时可选开启 `proxy_ssl_verify` 校验，防回源链路被中间人）。目标可带路径前缀（`http://127.0.0.1:8080/api`，整站请求挂到后端该路径下，尾斜杠自动补齐）；从浏览器地址栏整条粘贴时，`?查询` 与 `#锚点` 会被自动剥掉（`#` 是 nginx 的注释符，混进配置会直接导致语法错误），末段像具体页面（`/management.html`）时会提醒你多半该只填到端口。一个域名对应一份配置、一个后端；同域名下按路径分流到多个后端暂不支持。
...
- **运维入口**：nginx 运行状态、`nginx -t`、平滑重载 / 强制生效、错误与访问日志速览、本脚本操作记录。
- **源站换 IP 后立即收敛**：主菜单「应用配置并强制生效」会先清本机 DNS 缓存再重启 nginx，断开全部旧连接、重新解析所有回源域名。`reload` 做不到这点——nginx 只在配置加载那一刻解析一次 upstream 域名，且旧 worker 会带着到旧 IP 的长连接继续服务；需要不断连时才用运维菜单里的平滑重载。
...
2. 管理反向代理
   ...
   └─ 运行状态 / 日志 / 平滑重载 reload · 强制生效 restart
3. 安装 Nginx
4. 更新 Nginx 程序
5. 更新本脚本（拉 GitHub 最新）
6. 应用配置并强制生效（断开所有连接，回源域名重新解析 IP）
9. 卸载 Nginx（危险）
0. 退出
```
